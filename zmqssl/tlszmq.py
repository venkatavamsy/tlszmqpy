...
class TLSZmq(object):
    ...
    def init_ssl(self):
        ...
        if self.type == 'Server':
            ...
            self.ctx.set_session_id_ctx(self.identity.encode(errors='surrogateescape'))
            self.ssl.set_session_id_ctx(self.identity.encode(errors='surrogateescape'))
            self.ssl.set_accept_state()
        else:
            self.ssl.set_connect_state()

    def update(self):
        if self.zmq_to_ssl.tell():
            rc = self.rbio.write(self.flush(self.zmq_to_ssl))
            self.LOG.debug('%s written to BIO' % (rc))
        if self.app_to_ssl.tell():
            rc = self.ssl.write(self.app_to_ssl.getvalue().encode(errors='surrogateescape'))
            if not self.continue_ssl(rc):
                raise Exception('SSL Error')
            if rc == self.app_to_ssl.tell():
    ...
    def net_write(self):
        while True:
            ...
            if read is None:
                break
            # print(type(read))
            self.ssl_to_zmq.write(read.decode(errors='surrogateescape'))
        if self.ssl_to_zmq.tell():
            self.LOG.debug("%s read from BIO" % (self.ssl_to_zmq.tell()))

    def can_recv(self):
        return self.ssl_to_app.tell()

    def needs_write(self):
        return self.ssl_to_zmq.tell()
    ...
    def put_data(self, data):
        self.zmq_to_ssl.write(data.decode(errors='surrogateescape'))
